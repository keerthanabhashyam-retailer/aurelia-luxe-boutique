...
const SpecialRequest: React.FC<SpecialRequestProps> = ({ onSync }) => {
  ...
  const handleSubmit = async () => {
    ...
    const requestData = { ...form, image: photo, timestamp: Date.now() };
    if (onSync) await onSync(requestData);
    
    alert("Your luxury request has been submitted to our artisans and logged to Google Drive/Sheets.");
    setStep(1);
    setPhoto(null);
      ...
      <div className="bg-white rounded-[2.5rem] p-10 md:p-14 border border-stone-100 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="text-center mb-12">
            ...
            <p className="text-stone-400 max-w-md mx-auto text-sm leading-relaxed">Collaborate with our master craftsmen to create a unique masterpiece.</p>
          </div>
          ...
          {step === 1 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
              <div className="space-y-4">
                ...
                <textarea placeholder="Describe your dream piece..." className="w-full p-6 bg-stone-50 border border-stone-200 rounded-3xl min-h-[180px]" value={form.description} onChange={e => setForm({...form, description: e.target.value})} />
              </div>
          ...
          {step === 2 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <label className="block text-[10px] font-bold text-stone-400 uppercase tracking-widest">Visual References</label>
                  <div className="relative aspect-square bg-stone-50 rounded-3xl border-2 border-dashed border-stone-200 flex flex-col items-center justify-center overflow-hidden group">
                    {photo ? (
                      ...
                      <div className="flex flex-col items-center gap-4 p-8 text-center">
                        <Icons.Camera />
                        <p className="text-[10px] text-stone-400 uppercase tracking-widest">Sync directly to Google Drive</p>
                ...
                <div className="space-y-6">
                  <div className="space-y-4">
                    ...
                    <select className="w-full p-5 bg-stone-50 border border-stone-200 rounded-2xl" value={form.view} onChange={e => setForm({...form, view: e.target.value})}><option>Traditional / Classic</option><option>Modern / Avant-Garde</option></select>
                  </div>
                  <div className="space-y-4">
                    <label className="block text-[10px] font-bold text-stone-400 uppercase tracking-widest">Order Volume</label>
                    <input type="number" min="1" className="w-full p-5 bg-stone-50 border border-stone-200 rounded-2xl" value={form.quantity} onChange={e => setForm({...form, quantity: Number(e.target.value)})} />
                  </div>
                </div>
              </div>
              <div className="flex gap-4">
                <button onClick={() => setStep(1)} className="flex-1 bg-stone-100 text-stone-600 py-5 rounded-2xl font-bold">Return</button>
                <button onClick={() => setStep(3)} className="flex-[2] bg-stone-900 text-white py-5 rounded-2xl font-bold hover:bg-amber-600 shadow-xl">Confirm Details</button>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
              <input type="date" className="w-full p-6 bg-stone-50 border border-stone-200 rounded-3xl" value={form.dueDate} onChange={e => setForm({...form, dueDate: e.target.value})} />
              <div className="flex gap-4">
